...
from usecases.agenticrag.settings import AZURE_AI_FOUNDRY_SHAREPOINT_AGENT, AZURE_AI_FOUNDRY_FABRIC_AGENT, AZURE_AI_FOUNDRY_WEB_AGENT
...
class PromptManager:
    ...
    def create_user_prompt_planner(self, user_query: str, **context) -> str:
        ...
        return self.get_prompt(
            ...
            fabric_agent=context.get("fabric_agent", self.AZURE_AI_FOUNDRY_FABRIC_AGENT),
            sharepoint_agent=context.get("sharepoint_agent", self.AZURE_AI_FOUNDRY_SHAREPOINT_AGENT),
            web_agent=context.get("web_agent", self.AZURE_AI_FOUNDRY_WEB_AGENT),
            **{k: v for k, v in context.items() if k not in ["fabric_agent", "sharepoint_agent", "web_agent"]}
        )
